...
This survey completed `r coastaltows` coastal LFS 7742 trawl net tows (Appendix \@ref(app:appBridge)). The coastal LFS 7742 trawl net averaged `r coastalhigh` m high by `r coastalwide` m wide, or an area of `r round(coastalhigh * coastalwide)` m^2^, during surface tows on this survey. This mouth area is larger than expected, but similar to a previous survey on the *FV Sea Crest* where the coastal LFS 7742 trawl net mouth opening averaged 19 m high by 41 m wide at the surface, or area of (779 m^2^; [@anderson2019]). All catch per unit effort (CPUE) reported here are standardized by swept volume to standardize catch values.
...
Samples were collected for DNA stock composition (`r numDNA`), otoliths (`r numOtoliths`), energy density (`r numWholebodies`), stable isotope analysis (`r numMuscles`), and coded wire tags (`r numCTW`). These biological samples are located at the Pacific Biological Station, Fisheries and Oceans Canada (Nanaimo, BC) awaiting laboratory analysis.
...
Vertical bongo tows were conducted at `r nrow(planktonvert)` stations to depths ranging from `r planktonmin` m to `r planktonmax` m (Appendix \@ref(app:appOcean)). Formalin-preserved zooplankton samples have been identified and enumerated at the Institute of Ocean Sciences, Fisheries and Oceans Canada (Sidney, BC). Data is archived in the zooplankton database. Fractionated zooplankton samples are frozen at the Pacific Biological Station, Fisheries and Oceans Canada (Nanaimo, BC).
